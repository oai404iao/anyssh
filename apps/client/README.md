...
The terminal workspace supports up to eight independent Session Tabs. Frontend Tab ID,
Connection Generation, and Rust Session ID are separate; each Tab owns an xterm.js
instance, status, terminal size, Host Key state, and authentication challenge state.
Inactive terminals remain mounted and continue output acknowledgement, while only the
visible terminal is fitted. Disconnect preserves the Tab and Scrollback; Close removes
only that Tab; Vault Lock and application exit disconnect all Sessions.
